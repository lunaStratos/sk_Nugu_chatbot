...
//2019.08.01 [라이브코드]
//기존 최신로또 당첨번호를 파싱 형태에서 api의 카운트 형태를 바꾸었습니다.
...
exports.nugu_lotto = (req, res) => {
  ...
  function getLottoJson(num, callback) {
    ...
    if (num != 0) { //num의 숫자를 이용해서 현재회차인지, 이전회차인지 구별합니다.
      url = "https://dhlottery.co.kr/common.do?method=getLottoNumber&drwNo=" + num;
    } else { // 최신 회차는 크롤링을 통해서 구현

      //20시 50분 추첨
    let nowNum = new Date()
    nowNum.setFullYear(2019, 2 - 1, 9)
    nowNum.setHours(20)
    nowNum.setMinutes(55)

    let count = 845 // 845회차를 기준으로 설정
    //구글 GCP상의 시간으로 이동
    let nowDate = new Date(new Date().getTime() + (540 * 60 * 1000));
    //week count. Math.floor로 소수점은 무조건 버림.
    let weekCount = parseInt(Math.floor((nowDate.getTime() - nowNum.getTime()) / (3600000 * 24 * 7)));
	count = count + weekCount;
    url = "https://dhlottery.co.kr/common.do?method=getLottoNumber&drwNo=" + count;
    } // if
