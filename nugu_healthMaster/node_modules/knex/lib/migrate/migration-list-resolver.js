...
var DEFAULT_LOAD_EXTENSIONS = exports.DEFAULT_LOAD_EXTENSIONS = (0, _freeze2.default)(['.co', '.coffee', '.eg', '.iced', '.js', '.litcoffee', '.ls', '.ts']);

// Lists all available migration versions, as a sorted array.
function listAll(absoluteConfigDir) {
  var loadExtensions = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : DEFAULT_LOAD_EXTENSIONS;

  return _bluebird2.default.promisify(_fs2.default.readdir, { context: _fs2.default })(absoluteConfigDir).then(function (migrations) {
...
function listAllAndCompleted(config, trxOrKnex, absoluteConfigDir) {
  return _bluebird2.default.all([listAll(absoluteConfigDir, config.loadExtensions), listCompleted(config.tableName, config.schemaName, trxOrKnex)]);
}